               ...
               <Grid gridTemplateColumns="auto" gridTemplateRows="min-content">
                  <UIGridContainer withoutOverflow gridTemplateColumns={mainGridTemplateColumns} gridTemplateRows="auto">
                     ...
                     <Grid
                        ...
                        gridTemplateRows={`repeat(${columns.length + 1}, ${ROW_HEIGHT}px) ${compactView ? 0 : ROW_HEIGHT}px ${rowsFillData.map(fillData => `${fillData ? ROW_HEIGHT : 0}px`).join(' ')}`}
                     >
                        <ColumnsHeader
                           compactView={compactView}
                           metrics={metrics}
                        ...
                        {data.map((item, index) => (
                           ...
                           <GridItem
                              ...
                              hidden={!(columnsFillData[index % numberOfColumns] && rowsFillData[Math.floor(index / numberOfColumns)])}
                           >
